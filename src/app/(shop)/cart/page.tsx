      ...
      <main className="pt-24 min-h-screen bg-text-primary">
        <div className="max-w-4xl mx-auto px-6 py-12">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-8">
            Il tuo carrello {items.length > 0 && `(${items.length})`}
          </h1>

          {items.length === 0 ? (
            <div className="text-center py-20 bg-white/5 border border-white/10 rounded-3xl">
              <ShoppingBag size={48} className="text-gray-600 mx-auto mb-4" />
              <p className="text-gray-400 mb-6">Il carrello è vuoto</p>
              <Link href="/shop">
                <Button variant="secondary">Esplora la collezione</Button>
              </Link>
            </div>
          ) : (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 h-fit">
                <h2 className="text-lg font-bold text-white mb-4">Riepilogo</h2>
                <div className="flex justify-between text-gray-300 text-sm mb-2">
                  <span>Subtotale</span>
                  <span>€{totalPrice().toFixed(2)}</span>
                </div>
                <p className="text-gray-500 text-xs mb-4">Spedizione calcolata al checkout</p>
                ...
                <div className="flex flex-col gap-1.5 mt-4 text-gray-500 text-xs">
                  <span className="flex items-center gap-1.5"><Truck size={13} /> Spedizione rapida</span>
                  <span className="flex items-center gap-1.5"><RotateCcw size={13} /> Reso entro 14 giorni</span>
                  <span className="flex items-center gap-1.5"><ShieldCheck size={13} /> Pagamento sicuro</span>
                </div>
              </div>
            </div>
          )}
