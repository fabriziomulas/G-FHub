          ...
          <nav className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-3 py-2 text-sm text-text-secondary hover:text-text-primary hover:bg-text-primary/5 rounded-lg transition-all"
              >
                {link.label}
              </Link>
            ))}
            ...
            {user ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-2 text-sm text-text-secondary hover:text-text-primary hover:bg-text-primary/5 rounded-lg transition-all"
                >
                  <User size={16} />
                ...
                <AnimatePresence>
                  {accountOpen && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -8 }}
                      className="absolute right-0 top-full mt-2 w-56 bg-white rounded-xl shadow-lg border border-text-primary/10 p-1 z-50"
                    >
                      ...
                      {user.role === "ADMIN" && (
                        <Link
                          href="/admin"
                          onClick={() => setAccountOpen(false)}
                          className="flex items-center gap-2 px-3 py-2 text-sm text-accent-electric hover:text-accent-purple hover:bg-text-primary/5 rounded-lg transition-all w-full"
                        >
                          Admin
                        </Link>
                      )}
                      <button
                        ...
                        className="flex items-center gap-2 px-3 py-2 text-sm text-red-400 hover:bg-red-50 rounded-lg transition-all w-full mt-1"
                      >
                        <LogOut size={14} /> Esci
                      </button>
                    </motion.div>
        ...
        <AnimatePresence>
          {mobileOpen && (
            <motion.div
              ...
              className="md:hidden bg-white border-t border-text-primary/10 mt-3"
            >
              <div className="px-6 py-4 flex flex-col gap-1">
                ...
                {user ? (
                  <>
                    ...
                    <Link href="/account" onClick={() => setMobileOpen(false)} className="text-text-secondary hover:text-text-primary py-2">
                      Dashboard
                    </Link>
                    {user.role === "ADMIN" && (
                      <Link href="/admin" onClick={() => setMobileOpen(false)} className="text-accent-electric py-2">
                        Admin
                      </Link>
                    )}
                  ...
                  <Link href="/account/login" onClick={() => setMobileOpen(false)} className="text-text-secondary hover:text-text-primary py-2">
                    Accedi
                  </Link>
                )}
